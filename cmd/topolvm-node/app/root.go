...
func init() {
	...
	fs.BoolVar(&config.volumeHealth, "enable-volume-health", true, "Report the volume condition in NodeGetVolumeStats. It runs LVM commands for every volume on every stats request, and kubelet uses the result only with the CSIVolumeHealth feature gate enabled.")
