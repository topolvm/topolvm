...
func getLVMState(ctx context.Context) ([]vg, []lv, error) {
	args := []string{
		optReportFormat, reportFormatJSON,
		optUnits, "b", optNoSuffix,
		optConfigReport, "vg", "-o", "vg_name,vg_uuid,vg_size,vg_free",
		optConfigReport, "lv", "-o", "lv_uuid,lv_name,lv_full_name,lv_path,lv_size," +
			"lv_kernel_major,lv_kernel_minor,origin,origin_size,pool_lv,lv_tags," +
			"lv_attr,vg_name,data_percent,metadata_percent,pool_lv",
		optConfigReport, "pv", optNoFields,
		optConfigReport, "pvseg", optNoFields,
		optConfigReport, "seg", optNoFields,
	}
