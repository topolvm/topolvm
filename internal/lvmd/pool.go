...
type storagePool interface {
	Free(context.Context) (uint64, error)
	ListVolumes(context.Context) (map[string]*command.LogicalVolume, error)
	FindVolume(context.Context, string) (*command.LogicalVolume, error)
...
func (p *thinPoolAdapter) Free(ctx context.Context) (uint64, error) {
	...
	return usage.Free(p.overprovisionRatio)
}
