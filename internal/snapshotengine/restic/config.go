...
func NewResticWrapper(options config.SetupOptions) (*ResticWrapper, error) {
...
func NewResticWrapperFromShell(options config.SetupOptions, sh *shell.Session) (*ResticWrapper, error) {
	wrapper := &ResticWrapper{
		sh:     sh,
		config: options,
	}
...
func (w *ResticWrapper) SetCombineOutput(combine bool) {
	w.config.CombineOutput = combine
}
...
func (w *ResticWrapper) SetStdErr(stdBuffer bytes.Buffer) {
	w.sh.Stderr = &stdBuffer
}
