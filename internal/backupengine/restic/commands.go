...
func (w *ResticWrapper) validateConnections() ([]byte, error) {
	klog.Infoln("Validating connections to the backend....")
	args := w.appendCacheDirFlag([]interface{}{"snapshots", "--json", "--no-lock"})
	args = w.appendCaCertFlag(args)
	args = w.appendInsecureTLSFlag(args)
	args = w.appendMaxConnectionsFlag(args)
...
func (w *ResticWrapper) run(commands ...Command) ([]byte, error) {
	...
	var out []byte
	if w.config.CombineOutput {
		out, err = w.sh.CombinedOutput()
	} else {
		out, err = w.sh.Output()
	}
