...
func (w *ResticWrapper) setupEnv() error {
	...
	if err := w.setupTempDir(); err != nil {
		return err
	}
	if err := w.setupCACert(); err != nil {
		return err
	}
	if err := w.setupCache(); err != nil {
		return err
	}
	return w.setupProvider()
}

func (w *ResticWrapper) setupTempDir() error {
...
func (w *ResticWrapper) setupCACert() error {
...
func (w *ResticWrapper) setupCache() error {
...
func (w *ResticWrapper) setupProvider() error {
	switch w.config.Provider {
	case string(v1.ProviderS3):
		return w.setupS3()
	case string(v1.ProviderGCS):
		return w.setupGCS()
	case string(v1.ProviderAzure):
		return w.setupAzure()
	case string(v1.ProviderSwift):
		return w.setupSwift()
	case string(v1.ProviderB2):
		return w.setupB2()
	case string(v1.ProviderRest):
		return w.setupRest()
	}
	return nil
}

func (w *ResticWrapper) setupS3() error {
...
func (w *ResticWrapper) setupGCS() error {
...
func (w *ResticWrapper) setupAzure() error {
	...
	return w.exportSecretKey(AZURE_ACCOUNT_KEY, false)
}

func (w *ResticWrapper) setupSwift() error {
	r := fmt.Sprintf("swift:%s:/%s", w.config.Bucket, w.config.Path)
	w.sh.SetEnv(RESTIC_REPOSITORY, r)

	return w.setupSwiftAuth()
}

func (w *ResticWrapper) setupSwiftAuth() error {
	// For keystone v1 authentication
	...
	// For keystone v2 authentication
	...
	// For keystone v3 authentication
	...
	// For keystone v3 application credential authentication
	...
	return w.exportSecretKey(OS_AUTH_TOKEN, false)
}

func (w *ResticWrapper) setupB2() error {
	...
	return w.exportSecretKey(B2_ACCOUNT_KEY, true)
}

func (w *ResticWrapper) setupRest() error {
